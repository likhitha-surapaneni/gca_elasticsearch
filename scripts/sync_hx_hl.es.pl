...
if (!scalar @to_es_host) {
  @to_es_host = ('wp-p2m-3a', 'wp-p1m-a1');
}
...
foreach my $to_es_host (@to_es_host) {
  $es_to{$to_es_host} = Search::Elasticsearch->new(nodes => "$to_es_host:9200", client => '1_0::Direct', request_timeout => 300)
}
my $es_from = Search::Elasticsearch->new(nodes => "$from_es_host:9200", client => '1_0::Direct', request_timeout => 300);

my ($sec,$min,$hour,$day,$month,$year) = localtime();
my $datestamp = sprintf('%04d%02d%02d_%02d%02d%02d', $year+1900, $month+1, $day, $hour, $min, $sec);
my $snapshot_name = 'sync_hx_hl_'.$datestamp;
...
gca_elasticsearch/scripts/sync_hx_hl.es.pl

=head1 SYNOPSIS

perl gca_elasticsearch/scripts/sync_hx_hl.es.pl --repo hipsci_repo --snap_index hipsci
...
We have 3 running elasticsearch instances: Hinxton staging (np2), Harlow production (p1m), Hinxton fallback (p2m)
...
Next, we rsync the snapshot to p2m/p1m, and restore the index in p2m/p1m, so all three servers are serving the same content to users.
...
The name of the staging server in which you have built your index: default is wp-np2-1a

=item -to_es_host

Name of the production servers which you sync TO: defaults to wp-p1m-a1, wp-p2m-3a
...
The /path/to/staging/disk should be read-write accessible to the unix user running elasticsearch (w3_vg_igsr01)
...
to the user who runs THIS script (vg_igsr_adm)
